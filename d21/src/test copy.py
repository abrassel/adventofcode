...
def find_button_sequence_length(code: str, robots: int) -> int:
    ...
    find_seq_cache = {}
    def find_sequence_len(code_s: str, phase: int):
        ...
        for cc in code_s:
            ...
            for path in paths:
                ...
                min_path_tmp = path
                if phase < robots:
                    min_path_len_tmp, min_path_tmp = find_sequence_len(path, phase + 1)
        ...
        find_seq_cache[(code_s, phase)] = (final_path_len, final_path)
        return (final_path_len, final_path)
...
def solver(data: ParsedData, part: int) -> int:
    ...
    for code in data.codes:
        moves_len, path = find_button_sequence_length(code, robots=2 if part == 1 else 25)
        print(path)
